...
%if %{with tests}
# Two tests consistently fail on aarch64 which cause the entire suite to fail,
# they pass on other arch's however.
# NB: Upstream as unmerged MR's that may fix those fails, until they get merged
# and we can re-check the check, disable tests on aarch64.
%ifnarch aarch64
%check
